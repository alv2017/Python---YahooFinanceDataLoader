...
class Test_GetAssetsFromCSV(unittest.TestCase):
    ...
    def test_returns_list_of_assets(self):  
        """
            Description: the function reads the assets from the
            existing csv file. In case of succes we expect to 
            get the assets from the file: ['AAPL', 'IBM', 'MSFT']
        """
    ...
    def test_raise_error_if_file_not_exists(self):
        """
            Description: if we try to read the assets from nonexistent file,
            error is raised
        """
